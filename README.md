...
팀 계획서(`DevOps 팀과제_1주차.pdf`)에 적힌 항목을 기준으로 지금 뭐가 실제로 되어 있는지 정리한 표입니다.

### 기술스택 (계획서 6페이지)

| 구분 | 계획서 | 실제 구현 |
|---|---|---|
| 사용자 화면 | React + Vite + **TypeScript** | React + Vite + **JavaScript** — ⚠️ TS 아님, 팀 결정 필요 |
| 애플리케이션 | FastAPI | ✅ FastAPI |
| 데이터 저장 | PostgreSQL | ✅ PostgreSQL 16 |
| 배포 환경 | Docker Compose 기반 **팀 공용 Linux 환경** 배포 | ⚠️ Docker Compose는 됨, 지금은 개인 WSL 로컬 — 팀 공용 서버/VDI 배포는 아직 |
| CI/CD | GitHub–Jenkins 연동 | ⚠️ GitHub 저장소는 만듦, **Jenkins 파이프라인은 미착수** (계획상 3주차부터) |
| 관측·성능 | Prometheus/Grafana, Loki, JMeter | ❌ 미적용 (계획상 9~11주차) |
...
| 공간 목록/상세 조회 | ✅ |
| 날짜별 예약 가능 여부 조회 | ✅ |
| 예약 생성/취소 (중복 예약 차단) | ✅ PostgreSQL exclusion constraint |
| 내 예약 목록 | ✅ |
| 1인 일일 이용 한도 적용 | ✅ (동시성 레이스까지 막아둠) |
| 미입장 시 자동 취소 | ✅ |
| 관리자 기능 구현 | ⚠️ 부분 구현 — 강제 취소·이용 현황 조회·설정 변경·시간대 차단은 있음. **공간 운영시간 수정/휴무일 설정은 없음** (공간 생성만 가능) |
| 공간별 이용률 통계 | ❌ 미구현 (`/admin/stats`에 전체 집계는 있지만 공간별 %는 없음) |

### 핵심기술 과제 (계획서 10페이지 — 동일 슬롯 동시 예약)

| 요구사항 | 상태 |
|---|---|
| 동시 요청 2건 중 1건만 성공(201), 나머지는 409 | ✅ `test_concurrency.py`로 실측 검증 |

### 운영/프로세스 (계획서 2~4페이지 — Notion, Git Flow, Discord 등)

코드로 확인할 수 있는 부분이 아니라 표에서 제외했습니다. `main`/`develop` 브랜치 구조만 계획서(Git Flow)대로 맞춰뒀습니다.
